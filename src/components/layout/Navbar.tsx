    ...
    <>
      <motion.nav
        ...
        className={`fixed left-0 right-0 top-0 z-[100] transition-all duration-300 ${
          isScrolled
            ? "border-b border-white/10 bg-brand-dark/75 py-4 backdrop-blur-md"
            : "bg-transparent py-6"
        }`}
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 md:px-8">
            ...
            {LEFT_LINKS.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className="relative font-display text-xs font-semibold uppercase tracking-wider text-white hover:text-brand-orange transition-colors"
                >
                  {link.label}
                  {isActive && (
                    <motion.div
          ...
          <Link href="/" className="z-[110] flex items-center gap-2" data-hover="true">
            ...
            <span className="font-display text-xl font-black lowercase tracking-tighter text-white">
              austropical
            </span>
          </Link>
          ...
          <div className="hidden items-center gap-8 lg:flex">
            ...
            <div className="flex items-center gap-4 border-l border-white/10 pl-6">
              <Link href="/products" className="font-display text-xs font-semibold uppercase tracking-wider text-white/70 hover:text-white transition-colors">
                LOGIN
              </Link>
          ...
          <div className="flex items-center gap-4 lg:hidden">
            ...
            <button
              ...
              className="z-[110] text-white hover:text-brand-orange focus:outline-none"
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="fixed inset-0 z-[95] flex flex-col justify-between bg-brand-dark px-6 py-24 md:px-12"
          >
            ...
            >
              {allLinks.map((link) => (
                <motion.div key={link.href} variants={navLinkVariants}>
                  <Link
                    href={link.href}
                    className="font-display text-3xl font-extrabold uppercase text-white hover:text-brand-orange transition-colors"
                  >
                    {link.label}
                  </Link>
                </motion.div>
            ...
            >
              <div className="flex items-center gap-6">
                <Link
                  href="/products"
                  className="font-display text-lg font-bold uppercase text-white hover:text-brand-pink"
                >
                  LOGIN
                </Link>
                <Link
